...
class Wikis_Email_Cleaner_Validator {
    ...
    private function check_role_based_email( $email ) {
        $role_based = array(
            ...
            'help', 'info', 'contact', 'service', 'office', 'team', 'staff',
            'noreply', 'no-reply', 'donotreply', 'do-not-reply', 'abuse',
            'security', 'privacy', 'legal', 'billing', 'accounts', 'orders'
        );

        $local_part = strtolower( substr( $email, 0, strpos( $email, '@' ) ) );
        $is_role_based = in_array( $local_part, $role_based, true );
    ...
    private function apply_scoring_logic( $result ) {
        ...
        // Popular email providers get bonus points
        ...
        if ( isset( $result['validation_details']['domain']['valid'] ) &&
        ...
        $issue_count = count( $result['errors'] ) + count( $result['warnings'] );
        if ( $issue_count > 2 ) {
            $result['score'] -= ( $issue_count - 2 ) * 5;
        }
    ...
    /**
     * Determine email validity based on comprehensive analysis
     *
     * @param array $result Validation result
    ...
    private function determine_validity( $result ) {
        ...
        // Score-based determination with context
        $min_score = 60; // Default minimum score

        // Lower threshold for trusted providers
        if ( isset( $result['validation_details']['trusted_provider'] ) &&
             $result['validation_details']['trusted_provider'] ) {
            $min_score = 50;
        }

        // Higher threshold for suspicious patterns
        if ( isset( $result['validation_details']['disposable']['is_disposable'] ) &&
             $result['validation_details']['disposable']['is_disposable'] ) {
            $min_score = 80; // Very high bar for disposable emails
        }
    ...
    private function load_fake_patterns() {
        $this->fake_patterns = array(
            ...
            array(
                'name' => 'fake_patterns',
                'regex' => '/^(fake|dummy|invalid|null|void)[\d]*@/',
    ...
    private function get_default_disposable_domains() {
        return array(
            ...
            'spamgourmet.com', 'trashmail.com', '33mail.com',
            'mailcatch.com', 'mailnesia.com', 'tempail.com',
            'dispostable.com', 'tempinbox.com', 'minuteinbox.com',
            'emailtemporanea.com', 'temporaryemail.net', 'tempemails.net',

            // Burner email services
            'burnermail.io', 'guerrillamail.org', 'guerrillamail.net',
            'guerrillamail.biz', 'spam4.me', 'grr.la',
            'guerrillamail.de', 'trbvm.com', 'sharklasers.com',

            // Temporary services
            'mailtemp.info', 'tempmail.ninja', 'tempmail.email',
            'temp-mail.io', 'tempmailo.com', 'tempmailaddress.com',
            'tempmail.altmails.com', 'tempmail.plus',
    ...
    private function get_recommendations( $validation ) {
        ...
        if ( isset( $validation['validation_details']['disposable']['is_disposable'] ) &&
        ...
        if ( isset( $validation['validation_details']['role_based']['is_role_based'] ) &&
        ...
        if ( isset( $validation['validation_details']['typo']['has_typo'] ) &&
